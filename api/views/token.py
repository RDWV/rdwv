...
async def create_token_fido2_begin(auth_data: schemes.FIDO2Auth):  # pragma: no cover
    ...
    options, state = Fido2Server(PublicKeyCredentialRpEntity(name="Rdwv", id=auth_data.auth_host)).authenticate_begin(
...
async def create_token_fido2_complete(request: Request):  # pragma: no cover
    ...
    try:
        Fido2Server(PublicKeyCredentialRpEntity(name="Rdwv", id=auth_host)).authenticate_complete(
            state,
