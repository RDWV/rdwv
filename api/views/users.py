...
async def register_fido2(
    ...
    options, state = Fido2Server(PublicKeyCredentialRpEntity(name="Rdwv", id=auth_data.auth_host)).register_begin(
...
async def fido2_complete_registration(
    ...
    try:
        auth_data = Fido2Server(PublicKeyCredentialRpEntity(name="Rdwv", id=auth_host)).register_complete(state, data)
