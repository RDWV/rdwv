...
def parse_manifest(manifest):
    ...
    version_constraint = manifest["constraints"]["rdwv"]
    req = Requirement(f"rdwv{version_constraint}")
    if VERSION not in req.specifier:
        raise ValueError(
            f"Invalid manifest.json: plugin requires Rdwv version{version_constraint}, but current version is {VERSION}"
        )
    return manifest
