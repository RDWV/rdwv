...
def create_bash_script(settings):
    ...
    if git_repo != DOCKER_REPO_URL:
        script += 'export RDWVGEN_DOCKER_IMAGE="rdwv/docker-compose-generator:local"\n'
    script += f"export RDWV_HOST={domain}\n"
    if reverseproxy != "nginx-https":
        script += f"export RDWV_REVERSEPROXY={reverseproxy}\n"
    script += f"export RDWV_CRYPTOS={cryptos_str}\n"
    ...
    if installation_pack != "all":
        script += f"export RDWV_INSTALL={installation_pack}\n"
    if additional_components:
        script += f"export RDWV_ADDITIONAL_COMPONENTS={','.join(additional_components)}\n"
